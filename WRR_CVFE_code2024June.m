...
%NOTE THIS INCLUDES THE REVIDED BOUNDAY FOR WATER TABLE
...
%Computation ellaspe time on a macbook is ~5mins
...
for tstep=1:totstep %Main time loop
    ...
    %But when there is outflow at a surface node,
    %we need to reset the condition as follows:
    
   
    %nsub=cols; %go to top ode of each column
    nsub=size(Btopsea,2); %go over summerged nodes
    ...
    for ii=1:nsub
        ...
        %Thus we can i.e.,clculate the sign of the vertical discharge as
        ...
        if qvertsig>0 % we will have outflow and thus we set 
            ...
            %equal to the value of the node immediately below a Neumann
            %condition  
            %Note this setting is explicit, lagging behind one time step
        end
    end
    
%     %Water bondary as suggested by Rev 3
%     nwat=cols-nsub;
%     Btopwat=Btop(1:nwat);
%     for ii=1:nwat
%         bnode=Btopwat(ii);
